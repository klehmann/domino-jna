...
public class NotesDatabase implements IRecyclableNotesObject {
	 ...
	 * 
	 * @param session
	 *            session to read the name of the server the API is running on
	 * @param server
	 *            database server
	 * @param filePath
	 *            database filepath
	 ...
	 * @param filePath
	 *            database filepath
	 * @param asUserCanonical
	 *            user context to open database or null to run as server
	 */
	 ...
	 * @param filePath
	 *            database filepath
	 * @param namesForNamesList
	 *            optional names list for the user to open the database; same
	 *            content as @Usernameslist, but can be any combination of
	 *            names, groups or roles (does not have to exist in the
	 *            directory)
	 */
	 ...
	 *            optional names list
	 * @param asUserCanonical
	 *            user context to open database or null to run as server; will
	 *            be ignored if code is run locally in the Notes Client
	 */
	private NotesDatabase(Session session, String server, String filePath, List<String> namesForNamesList,
			String asUserCanonical) {
		m_session = session;
			...
			if (!"".equals(server)) {
				if (session.isOnServer()) {
					...
					if (nameServer.getCommon().equalsIgnoreCase(nameCurrServer.getCommon())) {
						// switch to "" as servername if server points to the
						// server the API is running on;
						// this enables advanced lookup features like using
						// NEXT_SELECTED in view traversal
						server = "";
					}
				}
			}
		...
		if (NotesJNAContext.is64Bit()) {
				...
				try {
					if (m_authenticateUser) {
						if (notesAPI instanceof WinNotesCAPI) {
							...
							namesList.read();
							// setting authenticated flag for the user is
							// required when running on the server
							namesList.Authenticated = NotesCAPI.NAMES_LIST_AUTHENTICATED
									+ NotesCAPI.NAMES_LIST_PASSWORD_AUTHENTICATED;
							namesList.write();
							namesList.read();
						} else {
					...
					result = notesAPI.b64_NSFDbOpenExtended(retFullNetPath, openOptions, m_namesList.getHandle64(),
			...
			if (/* !m_isOnServer || */ (StringUtil.isEmpty(m_asUserCanonical) && namesForNamesList == null)) {
				...
				try {
					...
					NotesTimeDate retDataModified = new NotesTimeDate();
					NotesTimeDate retNonDataModified = new NotesTimeDate();
					result = notesAPI.b32_NSFDbOpenExtended(retFullNetPath, openOptions, m_namesList.getHandle32(),
							modifiedTime, hDB, retDataModified, retNonDataModified);
					NotesErrorUtils.checkResult(result);
				} finally {
	 ...
	 *            current session
	 * @param serverName
	 *            server name, either canonical, abbreviated or common name
	 * @param filePath
	 *            filepath to database
	 * @param dbClass
	 *            specifies the class of the database created. See DB_CLASS for
	 *            classes that may be specified.
	 * @param forceCreation
	 *            controls whether the call will overwrite an existing database
	 *            of the same name. Set to TRUE to overwrite, set to FALSE not
	 *            to overwrite.
	 * @param options
	 *            database creation option flags. See DBCREATE_xxx
	 * @param encryption
	 *            encryption strength
	 * @param maxFileSize
	 *            optional. Maximum file size of the database, in bytes. In
	 *            order to specify a maximum file size, use the database class,
	 *            DBCLASS_BY_EXTENSION and use the option,
	 *            DBCREATE_MAX_SPECIFIED.
	 */
	public static void createDatabase(Session session, String serverName, String filePath, short dbClass,
			boolean forceCreation, short options, Encryption encryption, long maxFileSize) {
		NotesCAPI notesAPI = NotesJNAContext.getNotesAPI();
		...
		short result = notesAPI.NSFDbCreateExtended(fullPathMem, dbClass, forceCreation, options, encryptStrengthByte,
				maxFileSize);
		NotesErrorUtils.checkResult(result);
	}
	 ...
	 * 
	 * @return session
	 * @deprecated will be removed if {@link NotesCollection} can decode the
	 *             collation info without falling back to the legacy API
	 */
	@Deprecated
	Session getSession() {
		return m_session;
	...
	/**
	 * This function gets the given database's {@link NotesDbReplicaInfo}
	 * structure.<br>
	 * <br>
	 * This structure contains information that tells the Domino Replicator how
	 * to treat the database.<br>
	 ...
	 * Domino divides this interval into thirds, and for each third of the
	 * interval carries out what amounts to an incremental purge.<br>
	 * <br>
	 * These deleted document identifiers are sometimes called deletion stubs.
	 * <br>
	 * <br>
	 * The ".Cutoff" member is a {@link NotesTimeDate} value that is calculated
	 * by subtracting the Cutoff Interval (also called Purge Interval) from
	 * today's date.<br>
	 * <br>
	...
	/**
	 * This function sets the given database's {@link NotesDbReplicaInfo}
	 * structure.<br>
	 * <br>
	 * Use this function to set specific values, such as the replica ID, in the
	 * header data of a database.<br>
	 * To create a new replica copy of a given database, for example, you must
	 * first create the new database using the NSFDbCreate, then get the replica
	 * ID of the source database via {@link #getReplicaInfo()}, then set this
	 * replica ID into the new database via
	 * {@link #setReplicaInfo(NotesDbReplicaInfo)}.<br>
	 * <br>
	 * You may also use {@link #setReplicaInfo(NotesDbReplicaInfo)} to set
	 * values such as the replication flags in the header of the database.<br>
	 * <br>
	...
	/**
	 * Returns the hex encoded replica id of the database (16 character hex
	 * string)
	 * 
	 * @return replica id
	 */
	...
	public String toString() {
		if (isRecycled()) {
			...
			return "NotesDatabase [handle=" + (NotesJNAContext.is64Bit() ? m_hDB64 : m_hDB32) + ", server="
					+ getServer() + ", filepath=" + getRelativeFilePath() + "]";
		}
	}
	...
	private void loadPaths() {
		if (m_paths == null) {
			...
			String canonicalPathName = NotesStringUtils.fromLMBCS(retCanonicalPathName,
					NotesStringUtils.getNullTerminatedLength(retCanonicalPathName));
			String expandedPathName = NotesStringUtils.fromLMBCS(retExpandedPathName,
					NotesStringUtils.getNullTerminatedLength(retExpandedPathName));
			String relDbPath;
			String absDbPath;
	 ...
	 * Prevent recycling.
	 * 
	 * @deprecated internal framework method, do only use it if you know what
	 *             you are doing
	 */
	@Deprecated
	 ...
	 *            name of the view/collection
	 * @param openFlagSet
	 *            open flags, see {@link OpenCollection}
	 * @return collection
	 */
	 ...
	 * which can be useful to reduce database size (by externalizing view
	 * indices) and to let one Domino server index data of another one.
	 * 
	 * @param dbData
	 *            database containing the data to populate the collection
	 * @param viewName
	 *            name of the view/collection
	 * @return collection
	 */
	 ...
	 */
	public NotesCollection openCollectionByNameWithExternalData(NotesDatabase dbData, String viewName,
			EnumSet<OpenCollection> openFlagSet) {
		checkHandle();
	...
	/**
	 * Opens a collection by its view note id. This method lets you store the
	 * view in a separate database than the one containing the actual data,
	 * which can be useful to reduce database size (by externalizing view
	 * indices) and to let one Domino server index data of another one.
	 * 
	 * @param dataDb
	 ...
	 * @return collection
	 */
	NotesCollection openCollectionWithExternalData(NotesDatabase dataDb, String name, int viewNoteId,
			EnumSet<OpenCollection> openFlagSet) {
		checkHandle();
		...
		EnumSet<OpenCollection> openFlagSetClone = openFlagSet == null ? EnumSet.noneOf(OpenCollection.class)
				: openFlagSet.clone();
		...
		if (NotesJNAContext.is64Bit()) {
			...
			if (m_namesList == null) {
				// open view as server
				result = notesAPI.b64_NIFOpenCollection(m_hDB64, dataDb.m_hDB64, viewNoteId, openFlags,
						unreadTable.getHandle64(), hCollection, null, viewUNID, collapsedList, selectedList);
				NotesErrorUtils.checkResult(result);
			} else {
				// now try to open collection as this user
				result = notesAPI.b64_NIFOpenCollectionWithUserNameList(m_hDB64, dataDb.m_hDB64, viewNoteId, openFlags,
						unreadTable.getHandle64(), hCollection, null, viewUNID, collapsedList, selectedList,
			...
			newCol = new NotesCollection(this, hCollection.getValue(), name, viewNoteId, sViewUNID,
					new NotesIDTable(collapsedList.getValue()), new NotesIDTable(selectedList.getValue()), unreadTable,
					m_asUserCanonical);
		} else {
			...
			if (m_namesList == null) {
				result = notesAPI.b32_NIFOpenCollection(m_hDB32, dataDb.m_hDB32, viewNoteId, openFlags,
						unreadTable.getHandle32(), hCollection, null, viewUNID, collapsedList, selectedList);
				NotesErrorUtils.checkResult(result);
			} else {
				// now try to open collection as this user
				result = notesAPI.b32_NIFOpenCollectionWithUserNameList(m_hDB32, dataDb.m_hDB32, viewNoteId, openFlags,
						unreadTable.getHandle32(), hCollection, null, viewUNID, collapsedList, selectedList,
	...
	public int findCollection(String collectionName) {
		...
		short result;
		if (NotesJNAContext.is64Bit()) {
			result = notesAPI.b64_NIFFindDesignNoteExt(m_hDB64, viewNameLMBCS, NotesCAPI.NOTE_CLASS_VIEW,
					NotesStringUtils.toLMBCS(NotesCAPI.DFLAGPAT_VIEWS_AND_FOLDERS, true), viewNoteID, 0);
		} else {
			result = notesAPI.b32_NIFFindDesignNoteExt(m_hDB32, viewNameLMBCS, NotesCAPI.NOTE_CLASS_VIEW,
					NotesStringUtils.toLMBCS(NotesCAPI.DFLAGPAT_VIEWS_AND_FOLDERS, true), viewNoteID, 0);
		}
	 ...
	 *            fulltext query
	 * @param limit
	 *            Maximum number of documents to return. Use 0 to return the
	 *            maximum number of results for the search
	 * @param filterIDTable
	 *            optional ID table to further refine the search. Use null if
	...
	public SearchResult ftSearch(String query, short limit, NotesIDTable filterIDTable) {
		...
		if (NotesJNAContext.is64Bit()) {
			...
			result = notesAPI.b64_FTSearch(m_hDB64, rethSearch, 0, queryLMBCS, searchOptionsBitMask, limit,
					filterIDTable == null ? 0 : filterIDTable.getHandle64(), retNumDocs, new Memory(Pointer.SIZE), // Reserved
																													// field
					rethResults);
			NotesErrorUtils.checkResult(result);
			...
			return new SearchResult(rethResults.getValue() == 0 ? null : new NotesIDTable(rethResults.getValue()),
					retNumDocs.getValue());
		} else {
			...
			result = notesAPI.b32_FTSearch(m_hDB32, rethSearch, 0, queryLMBCS, searchOptionsBitMask, limit,
					filterIDTable == null ? 0 : filterIDTable.getHandle32(), retNumDocs, new Memory(Pointer.SIZE), // Reserved
																													// field
					rethResults);
			NotesErrorUtils.checkResult(result);
	 ...
	 * database, because it minimizes the network traffic by sending only one
	 * request to the Lotus Domino Server.<br>
	 * <br>
	 * Note: This function will return an error if the ID table contains View
	 * notes or Design notes.
	 * 
	 * @param idTable
	 *            ID table of Notes to be deleted
	 */
	...
	/**
	 * This function clears out the replication history information of the
	 * specified database replica. This can also be done using the Notes user
	...
	/**
	 * This function returns an ID Table of Note IDs of notes which have been
	 * modified in some way from the given starting time until "now". The ending
	 * time/date is returned, so that this function can be performed
	 * incrementally.<br>
	 * Except when TIMEDATE_MINIMUM is specified, the IDs of notes deleted
	 * during the time span will also be returned in the ID Table, and the IDs
	 * of these deleted notes have been ORed with {@link NotesCAPI#RRV_DELETED}
	 * before being added to the table. You must check the
	 ...
	 * specified time, the Notes C API returns an error ERR_NO_MODIFIED_NOTES.
	 * In our wrapper code, we check for this error and return an empty
	 * {@link NotesIDTable} instead.<br>
	 * <br>
	 * Note: You program is responsible for freeing up the returned id table
	 * handle.
	 * 
	 * @param noteClassMask
	 *            the appropriate NOTE_CLASS_xxx mask for the documents you wish
	 *            to select. Symbols can be OR'ed to obtain the desired Note
	 *            classes in the resulting ID Table.
	 * @param since
	 *            A TIMEDATE structure containing the starting date used when
	 *            selecting notes to be added to the ID Table built by this
	 *            function. To include ALL notes (including those deleted during
	 *            the time span) of a given note class, use
	 *            {@link NotesDateTimeUtils#setWildcard(NotesTimeDate)}. To
	 *            include ALL notes of a given note class, but excluding those
	 *            notes deleted during the time span, use
	 *            {@link NotesDateTimeUtils#setMinimum(NotesTimeDate)}.
	 * @param retUntil
	 *            A pointer to a {@link NotesTimeDate} structure into which the
	 *            ending time of this search will be returned. This can
	 *            subsequently be used as the starting time in a later search.
	 * @return newly allocated ID Table, you are responsible for freeing the
	 *         storage when you are done with it using
	 *         {@link NotesIDTable#recycle()}
	 */
	...
	public static abstract class SignCallback {
		 ...
		 * {@link #shouldSign(NotesViewEntryData, String)}. For performance
		 * reasons, the default implementation returns false.
		 * 
		 ...
		 * 
		 * @param noteData
		 *            note data from design collection
		 * @param currentSigner
		 *            current design element signer
		 * @return true to sign
		 */
	...
	public NotesAgent getAgent(String agentName) {
		...
		short result;
		if (NotesJNAContext.is64Bit()) {
			result = notesAPI.b64_NIFFindDesignNoteExt(m_hDB64, agentNameLMBCS, NotesCAPI.NOTE_CLASS_FILTER,
					NotesStringUtils.toLMBCS(NotesCAPI.DFLAGPAT_TOOLSRUNMACRO, true), retAgentNoteID, 0);
		} else {
			result = notesAPI.b32_NIFFindDesignNoteExt(m_hDB32, agentNameLMBCS, NotesCAPI.NOTE_CLASS_FILTER,
					NotesStringUtils.toLMBCS(NotesCAPI.DFLAGPAT_TOOLSRUNMACRO, true), retAgentNoteID, 0);
		}
		if (result == 1028) {
	...
	/**
	 * Sign all documents of a specified note class (see NOTE_CLASS_* in
	 * {@link NotesCAPI}.
	 * 
	 * @param noteClasses
	 *            bitmask of note classes to sign
	 * @param callback
	 *            optional callback to get notified about signed notes or null
	 */
	public void signAll(int noteClasses, SignCallback callback) {
		checkHandle();

		// TODO improve performance by checking first if the current signer has
		// already signed the documents (like in the legacy API)
		NotesCAPI notesAPI = NotesJNAContext.getNotesAPI();
		...
		try {
			...
			while (moreToDo) {
				boolean shouldReadSummaryDataFromDesignCollection = callback != null
						? callback.shouldReadSummaryDataFromDesignCollection() : false;
				...
				NotesViewLookupResultData data = col.readEntries(pos,
						isFirstRun ? EnumSet.of(Navigate.NEXT) : EnumSet.of(Navigate.CURRENT), isFirstRun ? 1 : 0,
						EnumSet.of(Navigate.NEXT), Integer.MAX_VALUE, readMask);
				...
				for (NotesViewEntryData currEntry : entries) {
					...
					if ((currNoteClass & noteClasses) != 0) {
						...
						if (NotesJNAContext.is64Bit()) {
							...
							short result = notesAPI.b64_NSFNoteOpen(m_hDB64, currNoteId,
									expandNote ? NotesCAPI.OPEN_EXPAND : 0, rethNote);
							NotesErrorUtils.checkResult(result);
							try {
								...
								result = notesAPI.b64_NSFNoteVerifySignature(rethNote.getValue(), null, retWhenSigned,
										retSigner, retCertifier);
								...
								if (result != 0) {
									...
									currNoteSigner = NotesStringUtils.fromLMBCS(retSigner,
											NotesStringUtils.getNullTerminatedLength(retSigner));
							...
							short result = notesAPI.b32_NSFNoteOpen(m_hDB32, currNoteId,
									expandNote ? NotesCAPI.OPEN_EXPAND : 0, rethNote);
							NotesErrorUtils.checkResult(result);
							try {
								NotesTimeDate retWhenSigned = new NotesTimeDate();
								...
								result = notesAPI.b32_NSFNoteVerifySignature(rethNote.getValue(), null, retWhenSigned,
										retSigner, retCertifier);
	 ...
	 * 
	 * @param options
	 *            Indexing options. See {@link FTIndex}
	 ...
	 * <br>
	 * This function does not disable full text indexing for a database. In
	 * order to disable full text indexing for a database, use
	 ...
	 * Internally calls {@link #getFTLastIndexTime()} and checks for null return
	 * value.
	 * 
	...
	/**
	 * This routine returns the last time a database was full text indexed. It
	 * can also be used to determine if a database is full text indexed. If the
	 * database is not full text indexed, null is returned.
	 * 
	...
	/**
	 * This function returns the "major" portion of the build number of the
	 * Domino or Notes executable running on the system where the specified
	 * database resides. Use this information to determine what Domino or Notes
	 * release is running on a given system. The database handle input may
	 * represent a local database, or a database that resides on a Lotus Domino
	 * Server.<br>
	 * <br>
	 * Domino or Notes Release 1.0 (all preliminary and final versions) are
	 * build numbers 1 to 81.<br>
	 * Domino or Notes Release 2.0 (all preliminary and final versions) are
	 * build numbers 82 to 93.<br>
	 * Domino or Notes Release 3.0 (all preliminary and final versions) are
	 * build numbers 94 to 118.<br>
	 * Domino or Notes Release 4.0 (all preliminary and final versions) are
	 * build numbers 119 to 136.<br>
	...
	/**
	 * This function returns a BUILDVERSION structure which contains all types
	 * of information about the level of code running on a machine.<br>
	 * <br>
	...
	/**
	 * This function scans all the notes in a database or files in a directory.
	 * <br>
	 * <br>
	 * Based on several search criteria, the function calls a user-supplied
	 * routine (an action routine) for every note or file that matches the
	 * criteria. NSFSearch is a powerful function that provides the general
	 * search mechanism for tasks that process all or some of the documents in a
	 * database or all or some of the databases in a directory.<br>
	 * <br>
	 * Specify a formula argument to improve efficiency when processing a subset
	 * of the notes in a database.<br>
	 * <br>
	 * In addition, the formula argument can be used to return computed
	 * "on-the-fly" information.<br>
	 * <br>
	 * To do this, you specify that a value returned from a formula is to be
	 * stored in a temporary field of each note.<br>
	 * <br>
	 * This temporary field and its value is then accessible in the summary
	 * buffer received by the NSFSearch action routine without having to open
	 * the note.<br>
	 * <br>
	 ...
	 * <br>
	 * In the action routine of NSFSearch, if you get a search match, look at
	 * the summary information.<br>
	 * The dLength field will be one of the items in the summary information
	 * buffer.<br>
	 * <br>
	 ...
	 * Specify the "since" argument to limit the search to notes created or
	 * modified in the database since a certain time/date.<br>
	 * When used to search a database, NSFSearch will search the database file
	 * sequentially if NULL is passed as the "Since" time.<br>
	 * If the search is not time-constrained (the "Since" argument is NULL or
	 * specifies the TIMEDATE_WILDCARD, ANYDAY/ALLDAY), then NSFSearch may find
	 * a given note more than once during the same search. If a
	 * non-time-constrained search passes a certain note to the action routine,
	 * and that note is subsequently updated, then NSFSearch may find that note
	 * again and pass it to the action routine a second time during the same
	 * search. This may happen if Domino or Notes relocates the updated note to
	 * a position farther down in the file. If your algorithm requires
	 * processing each note once and only once, then use time-constrained
	 * searches.<br>
	 * Save the return value of type {@link NotesTimeDate} of the present search
	 * and use that as the "Since" time on the next search.<br>
	 * <br>
	 * Alternatively, build an ID table as you search, avoid updating notes in
	 * the action routine, and process the ID table after the search completes.
	 * ID tables are guaranteed not to contain a given ID more than once.
	 * 
	 * @param formula
	 *            formula or null
	 * @param viewTitle
	 *            optional view title that will be returned for "@ ViewTitle"
	 *            within the formula or null
	 * @param searchFlags
	 *            flags to control searching ({@link Search})
	 * @param noteClassMask
	 *            bitmask of noteclasses to search
	 * @param since
	 *            The date of the earliest modified note that is matched. The
	 *            note's "Modified in this file" date is compared to this date.
	 *            Specify NULL if you do not wish any filtering by date.
	 * @param callback
	 *            callback to be called for every found note
	 * @return The ending (current) time/date of this search. Returned so that
	 *         it can be used in a subsequent call to
	 *         {@link #search(String, String, EnumSet, int, NotesTimeDate, ISearchCallback)}
	 *         as the "Since" argument.
	...
	public NotesTimeDate search(final String formula, String viewTitle, final EnumSet<Search> searchFlags,
			int noteClassMask, NotesTimeDate since, final ISearchCallback callback) throws FormulaCompilationError {
		checkHandle();
		...
		if (NotesJNAContext.is64Bit()) {
			...
			// not sure if this is necessary, but we had to use a special
			// library extending StdCallLibrary
			// for Windows and the documentation says this might be required for
			// callbacks as well
			// (StdCallCallback)
			if (notesAPI instanceof WinNotesCAPI) {
				apiCallback = new WinNotesCAPI.b64_NsfSearchProcWin() {
					...
					public void invoke(Pointer enumRoutineParameter, NotesSearchMatch64 searchMatch,
						...
						try {
							...
							if (searchFlags.contains(Search.SUMMARY)) {
								if (summaryBuffer != null) {
									...
									itemTableData = NotesLookupResultBufferDecoder.decodeItemTable(summaryBufferPtr,
											gmtOffset, isDST, convertStringsLazily);
								}
							}
							...
							callback.noteFound(NotesDatabase.this, noteId, noteClass, dbCreated, noteModified,
									itemTableData);
						} catch (Throwable t) {
							invocationEx[0] = t;
						}
			...
			if (!StringUtil.isEmpty(formula)) {
				// formulaName only required if formula is used for collection
				// columns
				...
				if (result == INotesErrorConstants.ERR_FORMULA_COMPILATION) {
					...
					throw new FormulaCompilationError(result, errMsg, formula, retCompileError.getValue(),
							retCompileErrorLine.getValue(), retCompileErrorColumn.getValue(),
							retCompileErrorOffset.getValue(), retCompileErrorLength.getValue());
				}
				NotesErrorUtils.checkResult(result);
			...
			try {
				...
				short result;
				if (m_namesList != null) {
					result = notesAPI.b64_NSFSearchWithUserNameList(m_hDB64, hFormula, viewTitleBuf, searchFlagsBitMask,
							(short) (noteClassMask & 0xffff), since, apiCallback, null, retUntil,
							m_namesList.getHandle64());
				} else {
					result = notesAPI.b64_NSFSearch(m_hDB64, hFormula, viewTitleBuf, searchFlagsBitMask,
							(short) (noteClassMask & 0xffff), since, apiCallback, null, retUntil);
			...
			// formulaName only required of formula is used for collection
			// columns
			int hFormula = 0;
			if (!StringUtil.isEmpty(formula)) {
				Memory formulaName = null;
			...
			try {
				...
				short result;
				if (m_namesList != null) {
					result = notesAPI.b32_NSFSearchWithUserNameList(m_hDB32, hFormula, viewTitleBuf, searchFlagsBitMask,
							(short) (noteClassMask & 0xffff), since, apiCallback, null, retUntil,
							m_namesList.getHandle32());
				} else {
					result = notesAPI.b32_NSFSearch(m_hDB32, hFormula, viewTitleBuf, searchFlagsBitMask,
							(short) (noteClassMask & 0xffff), since, apiCallback, null, retUntil);
				}
				NotesErrorUtils.checkResult(result);
	...
	public static class NoteInfo {
		...
		/**
		 * Returns the raw {@link NotesOriginatorId} object containing the data
		 * we also provide via direct methods
		 * 
		 * @return OID
		 */
	...
	/**
	 * Convenience to convert note ids to UNIDs. The method internally calls
	 * {@link NotesDatabase#getMultiNoteInfo(int[])}.
	 * 
	 * 
	 * @param noteIds
	 *            note ids to look up
	 * @param retUnidsByNoteId
	 *            map is populated with found UNIDs
	 * @param retNoteIdsNotFound
	 *            set is populated with any note id that could not be found
	 */
	...
	/**
	 * Get the note's the Originator ID (OID) structure, the time and date the
	 * note was last modified, the NOTE_CLASS_xxx, the time and date it was
	 * added to the database, the number of response documents and its parent's
	 * NoteID.
	 * 
	 * @param noteId
	 *            note id
	...
	public NoteInfoExt getNoteInfoExt(int noteId) {
		...
		if (NotesJNAContext.is64Bit()) {
			short result = notesAPI.b64_NSFDbGetNoteInfoExt(m_hDB64, noteId, retNoteOID, retModified, retNoteClass,
			...
			short result = notesAPI.b32_NSFDbGetNoteInfoExt(m_hDB32, noteId, retNoteOID, retModified, retNoteClass,
					retAddedToFile, retResponseCount, retParentNoteID);
			if (result == INotesErrorConstants.ERR_NOTE_DELETED) {
				isDeleted = true;
			} else if (result == INotesErrorConstants.ERR_INVALID_NOTE) {
	 ...
	 * the data returned by this method is the {@link NotesOriginatorId}, which
	 * contains the UNID of the document, the sequence number and the sequence
	 * time ("Modified initially" time).<br>
	 * <br>
	 * In addition, the method checks whether a document exists or has been
	 * deleted.<br>
	 * <br>
	 * Please note that the method can only handle max. 65535 note ids, because
	 * it's using a WORD / short datatype for the count internally to call the C
	 * API.
	 * 
	 * @param noteIds
	 *            array of note ids
	 ...
	 *             if note id array has too many entries (more than 65535)
	 */
	public NoteInfo[] getMultiNoteInfo(int[] noteIds) {
		...
		if (NotesJNAContext.is64Bit()) {
			...
			try {
				...
				result = notesAPI.b64_NSFDbGetMultNoteInfo(m_hDB64, (short) (noteIds.length & 0xffff), options,
						retHandleLong, retSize, rethOutBuf);
				NotesErrorUtils.checkResult(result);
				...
				if (retSizeLong != noteIds.length * entrySize) {
					throw new IllegalStateException(
							"Unexpected size of return data. Expected " + noteIds.length * entrySize
									+ " bytes for data of " + noteIds.length + " ids, got " + retSizeLong + " bytes");
				}
			...
			try {
				...
				result = notesAPI.b32_NSFDbGetMultNoteInfo(m_hDB32, (short) (noteIds.length & 0xffff), options,
						retHandleInt, retSize, rethOutBuf);
				NotesErrorUtils.checkResult(result);
	...
	/**
	 * Helper method to extract the return data of method
	 * {@link #getMultNoteInfo(int[])}
	 * 
	 * @param noteIds
	 *            note ids used for lookup
	 * @param outBufPtr
	 *            buffer pointer
	...
	/**
	 * This function reads a note into memory and returns a handle to the
	 * in-memory copy.<br>
	 * <br>
	 * If the note is marked as unread, by default this function does not change
	 * the unread mark.<br>
	 * You can use the {@link OpenNote#MARK_READ} flag to change an unread mark
	 * to read for remote databases.
	 * 
	 * @param noteIdStr
	 *            The note ID as hex string of the note that you want to open.
	 * @param openFlags
	 *            Flags that control the manner in which the note is opened.
	 *            This, in turn, controls what information about the note is
	 *            available to you and how it is structured. The flags are
	 *            defined in {@link OpenNote}.
	 * @return note
	 */
	 ...
	 * 
	 * @param noteId
	 *            The note ID of the note that you want to open.
	 * @param openFlags
	 *            Flags that control the manner in which the note is opened.
	 *            This, in turn, controls what information about the note is
	 *            available to you and how it is structured. The flags are
	...
	/**
	 * This function takes the Universal Note ID and reads the note into memory
	 * and returns a handle to the in-memory copy.<br>
	 * This function only supports the set of 16-bit WORD options described in
	 * the entry {@link OpenNote}.
	 * 
	 * @param unid
	 *            UNID
	 * @param openFlags
	 ...
	 * database blazing fast. If you move the application to another directory,
