...
public interface NotesCAPI extends Library {
	...
	/**
	 * Set if names list has been authenticated using external password --
	 * Triggers "maximum password access allowed" feature
	 */
	...
	short b32_NSFDbOpenExtended(Memory PathName, short Options, int hNames, NotesTimeDate ModifiedTime,
			IntBuffer rethDB, NotesTimeDate retDataModified, NotesTimeDate retNonDataModified);

	short b64_NSFDbOpenExtended(Memory PathName, short Options, long hNames, NotesTimeDate ModifiedTime,
			LongBuffer rethDB, NotesTimeDate retDataModified, NotesTimeDate retNonDataModified);
	...
	short b32_NSFDbSpaceUsageScaled(int dbHandle, IntByReference retAllocatedBytes, IntByReference retFreeBytes,
			IntByReference retGranularity);

	short b64_NSFDbSpaceUsageScaled(long dbHandle, IntByReference retAllocatedBytes, IntByReference retFreeBytes,
			IntByReference retGranularity);
	 ...
	 * NEVER be replicated. The former may be set and cleared by the Notes user
	 * interface. The latter is intended to be set programmatically and SHOULD
	 * NEVER be able to be cleared by the user interface.
	 * 
	 * The latter was invented to avoid having to set the replica ID to the
	 * known value of REPLICA_ID_NEVERREPLICATE. This latter method has the
	 * failing that DBs that use it cannot have DocLinks to them.
	 */
	...
	public short REPLFLG_USED_MASK = (short) ((0x4 | 0x8 | 0x10 | 0x40 | 0x80 | 0x100 | 0x200 | 0xC000 | 0x1000 | 0x2000
			| 0x4000) & 0xffff);

	/**
	 * Reserved ReplicaID.Date. Used in ID.Date field in ReplicaID to escape to
	 * reserved REPLICA_ID_xxx
	 */
	...
	public short b32_NSFDbReplicaInfoGet(int hDB, NotesDbReplicaInfo retReplicationInfo);

	public short b64_NSFDbReplicaInfoGet(long hDB, NotesDbReplicaInfo retReplicationInfo);

	public short b32_NSFDbReplicaInfoSet(int hDB, NotesDbReplicaInfo ReplicationInfo);

	public short b64_NSFDbReplicaInfoSet(long hDB, NotesDbReplicaInfo ReplicationInfo);

	short b32_NIFFindDesignNoteExt(int hFile, Memory name, short noteClass, Memory pszFlagsPattern, IntBuffer retNoteID,
			int Options);

	short b64_NIFFindDesignNoteExt(long hFile, Memory name, short noteClass, Memory pszFlagsPattern,
			IntBuffer retNoteID, int Options);

	short b32_NIFOpenCollection(int hViewDB, int hDataDB, int ViewNoteID, short OpenFlags, int hUnreadList,
			IntByReference rethCollection, IntByReference rethViewNote, Memory retViewUNID,
			IntByReference rethCollapsedList, IntByReference rethSelectedList);

	short b64_NIFOpenCollection(long hViewDB, long hDataDB, int ViewNoteID, short OpenFlags, long hUnreadList,
			LongByReference rethCollection, LongByReference rethViewNote, Memory retViewUNID,
			LongByReference rethCollapsedList, LongByReference rethSelectedList);

	short b32_NIFOpenCollectionWithUserNameList(int hViewDB, int hDataDB, int ViewNoteID, short OpenFlags,
			int hUnreadList, IntByReference rethCollection, IntByReference rethViewNote, Memory retViewUNID,
			IntByReference rethCollapsedList, IntByReference rethSelectedList, int nameList);

	short b64_NIFOpenCollectionWithUserNameList(long hViewDB, long hDataDB, int ViewNoteID, short OpenFlags,
			long hUnreadList, LongByReference rethCollection, LongByReference rethViewNote, Memory retViewUNID,
			LongByReference rethCollapsedList, LongByReference rethSelectedList, long nameList);

	short b32_NIFReadEntries(int hCollection, NotesCollectionPosition IndexPos, short SkipNavigator, int SkipCount,
			short ReturnNavigator, int ReturnCount, int ReturnMask, IntByReference rethBuffer,
	...
	short b64_NIFReadEntries(long hCollection, NotesCollectionPosition IndexPos, short SkipNavigator, int SkipCount,
			short ReturnNavigator, int ReturnCount, int ReturnMask, LongByReference rethBuffer,
			ShortByReference retBufferLength, IntByReference retNumEntriesSkipped, IntByReference retNumEntriesReturned,
			ShortByReference retSignalFlags);

	short b32_NIFFindByKey(int hCollection, Memory keyBuffer, short findFlags, NotesCollectionPosition retIndexPos,
			IntByReference retNumMatches);

	short b64_NIFFindByKey(long hCollection, Memory keyBuffer, short findFlags, NotesCollectionPosition retIndexPos,
			IntByReference retNumMatches);

	short b32_NIFFindByName(int hCollection, Memory name, short findFlags, NotesCollectionPosition retIndexPos,
			IntByReference retNumMatches);

	short b64_NIFFindByName(long hCollection, Memory name, short findFlags, NotesCollectionPosition retIndexPos,
			IntByReference retNumMatches);
	...
	short b32_NIFFindByKeyExtended2(int hCollection, Memory keyBuffer, int findFlags, int returnFlags,
			NotesCollectionPosition retIndexPos, IntByReference retNumMatches, ShortByReference retSignalFlags,
			IntByReference rethBuffer, IntByReference retSequence);

	short b64_NIFFindByKeyExtended2(long hCollection, Memory keyBuffer, int findFlags, int returnFlags,
			NotesCollectionPosition retIndexPos, IntByReference retNumMatches, ShortByReference retSignalFlags,
			LongByReference rethBuffer, IntByReference retSequence);

	short b32_NSFDbGetModifiedNoteTable(int hDB, short NoteClassMask, NotesTimeDate Since, NotesTimeDate retUntil,
			IntByReference rethTable);

	short b64_NSFDbGetModifiedNoteTable(long hDB, short NoteClassMask, NotesTimeDate Since, NotesTimeDate retUntil,
			LongByReference rethTable);

	short DNCanonicalize(int Flags, Memory TemplateName, Memory InName, Memory OutName, short OutSize,
			ShortByReference OutLength);

	short DNAbbreviate(int Flags, Memory TemplateName, Memory InName, Memory OutName, short OutSize,
			ShortByReference OutLength);
	...
	public void ODSWriteMemory(Pointer ppDest, short type, Pointer pSrc, short iterations);

	public void ODSReadMemory(Pointer ppSrc, short type, Pointer pDest, short iterations);
	...
	/**
	 * If the following is ORed in with a note class, the resultant note ID may
	 * be passed into NSFNoteOpen and may be treated as though you first did an
	 * NSFGetSpecialNoteID followed by an NSFNoteOpen, all in a single
	 * transaction.
	 */
	...
	/*
	 * If NOTE_CLASS_DEFAULT is ORed with another note class, it is in essence
	 * specifying that this is the default item in this class. There should only
	 * be one DEFAULT note of each class that is ever updated, although nothing
	 * in the NSF machinery prevents the caller from adding more than one. The
	 * file header contains a table of the note IDs of the default notes (for
	 * efficient access to them). Whenever a note is updated that has the
	 * default bit set, the reference in the file header is updated to reflect
	 * that fact. WARNING: NOTE_CLASS_DOCUMENT CANNOT have a "default". This is
	 * precluded by code in NSFNoteOpen to make it fast for data notes.
	 */
	 ...
	 */
	public static final short NOTE_CLASS_SINGLE_INSTANCE = (NOTE_CLASS_DESIGN | NOTE_CLASS_ACL | NOTE_CLASS_INFO
			| NOTE_CLASS_ICON | NOTE_CLASS_HELP_INDEX | 0);
	...
	/**
	 * If set, leave TYPE_RFC822_TEXT items in native format. Otherwise, convert
	 * to TYPE_TEXT/TYPE_TIME.
	 */
	public static final int OPEN_RAW_RFC822_TEXT = 0x01000000;
	/**
	 * If set, leave TYPE_MIME_PART items in native format. Otherwise, convert
	 * to TYPE_COMPOSITE.
	 */
	...
	/**
	 * Incremental type note Fully opened (NO place holders) This type of note
	 * is meant to retain the Item sequence numbers
	 */
	...
	/**
	 * At least one of the "definition" view items ($FORMULA, $COLLATION, or
	 * $FORMULACLASS) has been modified by another user since last ReadEntries.
	 * Upon receipt, you may wish to re-read the view note if up-to-date copies
	 * of these items are needed. Upon receipt, you may also wish to
	 * re-synchronize your index position and re-read the rebuilt index.<br>
	 * <br>
	 ...
	 * modified since last ReadEntries. Upon receipt, you may wish to re-read
	 * the view note if up-to-date copies of these items are needed.<br>
	 * <br>
	 * Signal returned only ONCE per detection
	 */
	...
	/**
	 * Collection index has been modified by another user since last
	 * ReadEntries. Upon receipt, you may wish to re-synchronize your index
	 * position and re-read the modified index.<br>
	 * <br>
	 * Signal returned only ONCE per detection
	 */
	 ...
	 * hCollection context Upon receipt, you may wish to repaint the window if
	 * the window contains the state of unread flags (This signal is never
	 * generated by NIF - only unread list users)
	 */
	...
	/**
	 * End of collection has not been reached due to buffer being too full. The
	 * ReadEntries should be repeated to continue reading the desired entries.
	 */
	public static final int SIGNAL_MORE_TO_DO = 0x0020;

	/**
	 * The view contains a time-relative formula (e.g., @Now). Use this flag to
	 * tell if the collection will EVER be up-to-date since time-relative views,
	 * by definition, are NEVER up-to-date.
	 */
	public static final int SIGNAL_VIEW_TIME_RELATIVE = 0x0040;

	/**
	 * Returned if signal flags are not supported This is used by
	 * NIFFindByKeyExtended when it is talking to a pre-V4 server that does not
	 * support signal flags for FindByKey
	 */
	public static final int SIGNAL_NOT_SUPPORTED = 0x0080;

	/**
	 * Mask that defines all "sharing conflicts", which are cases when the
	 * database or collection has changed out from under the user.
	 */
	public static final int SIGNAL_ANY_CONFLICT = (SIGNAL_DEFN_ITEM_MODIFIED | SIGNAL_VIEW_ITEM_MODIFIED
			| SIGNAL_INDEX_MODIFIED | SIGNAL_UNREADLIST_MODIFIED | SIGNAL_DATABASE_MODIFIED);

	/**
	 * Mask that defines all "sharing conflicts" except for
	 * SIGNAL_DATABASE_MODIFIED. This can be used in combination with
	 * SIGNAL_VIEW_TIME_RELATIVE to tell if the database or collection has truly
	 * changed out from under the user or if the view is a time-relative view
	 * which will NEVER be up-to-date. SIGNAL_DATABASE_MODIFIED is always
	 * returned for a time-relative view to indicate that it is never
	 * up-to-date.
	 */
	public static final int SIGNAL_ANY_NONDATA_CONFLICT = (SIGNAL_DEFN_ITEM_MODIFIED | SIGNAL_VIEW_ITEM_MODIFIED
			| SIGNAL_INDEX_MODIFIED | SIGNAL_UNREADLIST_MODIFIED);
	...
	public static final short OS_TRANSLATE_NATIVE_TO_LMBCS = 0; /*
																 ...
																 * LMBCS
																 */
	public static final short OS_TRANSLATE_LMBCS_TO_NATIVE = 1; /*
																 * Translate
																 * LMBCS to
																 * platform-
																 * specific
																 */
	public static final short OS_TRANSLATE_LOWER_TO_UPPER = 3; /*
																 * current int'l
																 * case table
																 */
	public static final short OS_TRANSLATE_UPPER_TO_LOWER = 4; /*
																 * current int'l
																 * case table
																 */
	public static final short OS_TRANSLATE_UNACCENT = 5; /*
															 * int'l unaccenting
															 * table
															 */
	...
	// WORD LNPUBLIC OSTranslate(WORD TranslateMode, const char far *In, WORD
	...
	public static short IDTABLE_MODIFIED = 0x0001; /*
													 * modified - set by
													 * Insert/Delete
													 */
	...
	public static long NOTEID_RESERVED = 0x80000000L; /*
														 * Reserved Note ID, used
														 * for categories in
														 * NIFReadEntries and
														 * for deleted notes in
														 * a lot of interfaces.
														 */

	public static long NOTEID_GHOST_ENTRY = 0x40000000L; /*
															 * Bit 30 -> partial
															 * thread ghost
															 * collection entry
															 */
	public static long NOTEID_CATEGORY = 0x80000000L; /*
														 * Bit 31 -> (ghost)
														 * "category entry"
														 */
	public static long NOTEID_CATEGORY_TOTAL = 0xC0000000L; /*
															 * Bit 31+30 ->
															 * (ghost)
															 * "grand total entry"
															 */
	public static long NOTEID_CATEGORY_INDENT = 0x3F000000L; /*
																 * Bits 24-29 ->
																 * category
																 * indent level
																 * within this
																 * column
																 */
	public static long NOTEID_CATEGORY_ID = 0x00FFFFFFL; /*
															 * Low 24 bits are
															 * unique category #
															 */

	public static long RRV_DELETED = NOTEID_RESERVED; /*
														 * indicates a deleted
														 * note (DBTABLE.C)
														 */
	...
	/**
	 * Guaranteed to be the greatest of Form, View or Macro length. NOTE: We
	 * need space for LEVELS-1 cascade characters and a NULL term. The +1 takes
	 * care of that.
	 */
	...
	public short b32_FTSearch(int hDB, IntByReference phSearch, int hColl, Memory query, int options, short limit,
			int hIDTable, IntByReference retNumDocs, Memory reserved, IntByReference rethResults);

	public short b64_FTSearch(long hDB, LongByReference phSearch, long hColl, Memory query, int options, short limit,
			long hIDTable, IntByReference retNumDocs, Memory reserved, LongByReference rethResults);
	...
	public short ConvertTIMEDATEToText(ByteBuffer intlFormat, ByteBuffer textFormat, NotesTimeDate inputTime,
			Memory retTextBuffer, short textBufferLength, ShortByReference retTextLength);

	public short ConvertTextToTIMEDATE(ByteBuffer intlFormat, ByteBuffer textFormat, Memory text, short maxLength,
	...
	public short ListGetText(ByteBuffer pList, boolean fPrefixDataType, short entryNumber, Memory retTextPointer,
			ShortByReference retTextLength);

	public short ListGetText(Pointer pList, boolean fPrefixDataType, short entryNumber, Memory retTextPointer,
			ShortByReference retTextLength);
	...
	public int DB_GETIDTABLE_VALIDATE = 0x00000001; /*
													 * If set, return only
													 * "validated" noteIDs
													 */

	public short b32_NSFFolderGetIDTable(int hViewDB, int hDataDB, int viewNoteID, int flags, IntByReference hTable);

	public short b64_NSFFolderGetIDTable(long hViewDB, long hDataDB, int viewNoteID, int flags, LongByReference hTable);

	public short b32_FolderDocAdd(int hDataDB, int hFolderDB, int FolderNoteID, int hTable, long dwFlags);

	public short b64_FolderDocAdd(long hDataDB, long hFolderDB, int FolderNoteID, long hTable, long dwFlags);

	public short b32_FolderDocCount(int hDataDB, int hFolderDB, int FolderNoteID, long dwFlags,
			LongByReference pdwNumDocs);

	public short b64_FolderDocCount(long hDataDB, long hFolderDB, int FolderNoteID, long dwFlags,
			LongByReference pdwNumDocs);

	public short b32_FolderDocRemove(int hDataDB, int hFolderDB, int FolderNoteID, int hTable, long dwFlags);

	public short b64_FolderDocRemove(long hDataDB, long hFolderDB, int FolderNoteID, long hTable, long dwFlags);

	public short b32_FolderDocRemoveAll(int hDataDB, int hFolderDB, int FolderNoteID, long dwFlags);

	public short b64_FolderDocRemoveAll(long hDataDB, long hFolderDB, int FolderNoteID, long dwFlags);

	public short b32_FolderMove(int hDataDB, int hFolderDB, int FolderNoteID, int hParentDB, int ParentNoteID,
			long dwFlags);

	public short b64_FolderMove(long hDataDB, long hFolderDB, int FolderNoteID, long hParentDB, int ParentNoteID,
			long dwFlags);

	public short b32_FolderRename(int hDataDB, int hFolderDB, int FolderNoteID, Memory pszName, short wNameLen,
			long dwFlags);

	public short b64_FolderRename(long hDataDB, long hFolderDB, int FolderNoteID, Memory pszName, short wNameLen,
			long dwFlags);
	...
	public void b32_NSFDbPathGet(int hDB, Memory retCanonicalPathName, Memory retExpandedPathName);

	public void b64_NSFDbPathGet(long hDB, Memory retCanonicalPathName, Memory retExpandedPathName);
	...
	public boolean b64_NSFNoteIsSignedOrSealed(long note_handle, ByteByReference signed_flag_ptr,
			ByteByReference sealed_flag_ptr);

	public boolean b32_NSFNoteIsSignedOrSealed(int note_handle, ByteByReference signed_flag_ptr,
			ByteByReference sealed_flag_ptr);

	public short b32_NSFNoteOpenByUNID(int hDB, NotesUniversalNoteId pUNID, short flags, IntByReference rethNote);

	public short b64_NSFNoteOpenByUNID(long hDB, NotesUniversalNoteId pUNID, short flags, LongByReference rethNote);

	public short b32_NSFNoteUnsign(int hNote);

	public short b64_NSFNoteUnsign(long hNote);

	public short b32_NSFItemInfo(int note_handle, Memory item_name, short name_len, NotesBlockId retbhItem,
			ShortByReference retDataType, NotesBlockId retbhValue, IntByReference retValueLength);

	public short b64_NSFItemInfo(long note_handle, Memory item_name, short name_len, NotesBlockId retbhItem,
			ShortByReference retDataType, NotesBlockId retbhValue, IntByReference retValueLength);
	...
	public short b32_NSFItemInfoNext(int note_handle, NotesBlockId.ByValue NextItem, Memory item_name, short name_len,
			NotesBlockId retbhItem, ShortByReference retDataType, NotesBlockId retbhValue,
			IntByReference retValueLength);

	public short b64_NSFItemInfoNext(long note_handle, NotesBlockId.ByValue NextItem, Memory item_name, short name_len,
			NotesBlockId retbhItem, ShortByReference retDataType, NotesBlockId retbhValue,
			IntByReference retValueLength);

	public short b32_NSFItemInfoPrev(int note_handle, NotesBlockId.ByValue CurrItem, Memory item_name, short name_len,
			NotesBlockId item_blockid_ptr, ShortByReference value_type_ptr, NotesBlockId value_blockid_ptr,
			IntByReference value_len_ptr);

	public short b64_NSFItemInfoPrev(long note_handle, NotesBlockId.ByValue CurrItem, Memory item_name, short name_len,
			NotesBlockId item_blockid_ptr, ShortByReference value_type_ptr, NotesBlockId value_blockid_ptr,
			IntByReference value_len_ptr);

	public short b64_NSFItemScan(long note_handle, NoteNsfItemScanProc ActionRoutine, Pointer funcParam);

	public short b32_NSFItemScan(int note_handle, NoteNsfItemScanProc ActionRoutine, Pointer funcParam);

	public void b32_NSFItemQueryEx(int note_handle, NotesBlockId.ByValue item_bid, Memory item_name,
			short return_buf_len, ShortByReference name_len_ptr, ShortByReference item_flags_ptr,
			ShortByReference value_datatype_ptr, NotesBlockId value_bid_ptr, IntByReference value_len_ptr,
			ByteByReference retSeqByte, ByteByReference retDupItemID);

	public void b64_NSFItemQueryEx(long note_handle, NotesBlockId.ByValue item_bid, Memory item_name,
			short return_buf_len, ShortByReference name_len_ptr, ShortByReference item_flags_ptr,
			ShortByReference value_datatype_ptr, NotesBlockId value_bid_ptr, IntByReference value_len_ptr,
			ByteByReference retSeqByte, ByteByReference retDupItemID);

	public short b32_NSFItemGetModifiedTime(int hNote, Memory ItemName, short ItemNameLength, int Flags,
			NotesTimeDate retTime);

	public short b64_NSFItemGetModifiedTime(long hNote, Memory ItemName, short ItemNameLength, int Flags,
			NotesTimeDate retTime);

	public short b32_NSFItemGetModifiedTimeByBLOCKID(int hNote, NotesBlockId.ByValue bhItem, int Flags,
			NotesTimeDate retTime);

	public short b64_NSFItemGetModifiedTimeByBLOCKID(long hNote, NotesBlockId.ByValue bhItem, int Flags,
			NotesTimeDate retTime);

	public short b32_NSFItemGetText(int note_handle, Memory item_name, Memory item_text, short text_len);

	public short b64_NSFItemGetText(long note_handle, Memory item_name, Memory item_text, short text_len);

	public short b64_NSFItemGetTextListEntries(long note_handle, Memory item_name);

	public short b32_NSFItemGetTextListEntries(int note_handle, Memory item_name);

	public short b64_NSFItemGetTextListEntry(long note_handle, Memory item_name, short entry_position,
			Memory retEntry_text, short text_len);

	public short b32_NSFItemGetTextListEntry(int note_handle, Memory item_name, short entry_position,
			Memory retEntry_text, short text_len);

	public short b32_NSFItemSetTextSummary(int hNote, Memory ItemName, Memory ItemText, short TextLength,
			boolean summary);

	public short b64_NSFItemSetTextSummary(long hNote, Memory ItemName, Memory ItemText, short TextLength,
			boolean summary);

	public boolean b32_NSFItemGetTime(int note_handle, Memory td_item_name, NotesTimeDate td_item_value);

	public boolean b64_NSFItemGetTime(long note_handle, Memory td_item_name, NotesTimeDate td_item_value);

	public short b32_NSFItemSetTime(int note_handle, Memory td_item_name, NotesTimeDate td_item_ptr);

	public short b64_NSFItemSetTime(long note_handle, Memory td_item_name, NotesTimeDate td_item_ptr);

	public boolean b32_NSFItemGetNumber(int hNote, Memory ItemName, DoubleByReference retNumber);

	public boolean b64_NSFItemGetNumber(long hNote, Memory ItemName, DoubleByReference retNumber);

	public long b32_NSFItemGetLong(int note_handle, Memory number_item_name, LongByReference number_item_default);

	public long b64_NSFItemGetLong(long note_handle, Memory number_item_name, LongByReference number_item_default);

	public short b32_NSFItemSetNumber(int hNote, Memory ItemName, double Number);

	public short b64_NSFItemSetNumber(long hNote, Memory ItemName, double Number);

	public short b64_NSFItemConvertToText(long note_handle, Memory item_name_ptr, Memory retText_buf_ptr,
			short text_buf_len, char separator);

	public short b32_NSFItemConvertToText(int note_handle, Memory item_name_ptr, Memory retText_buf_ptr,
			short text_buf_len, char separator);

	public short b64_NSFItemConvertValueToText(short value_type, NotesBlockId.ByValue value_bid, int value_len,
			Memory text_buf_ptr, short text_buf_len, char separator);

	public short b32_NSFItemConvertValueToText(short value_type, NotesBlockId.ByValue value_bid, int value_len,
			Memory text_buf_ptr, short text_buf_len, char separator);

	public short b64_NSFItemDelete(long note_handle, Memory item_name, short name_len);

	public short b32_NSFItemDelete(int note_handle, Memory item_name, short name_len);
	...
	public short b32_NSFDbGetMultNoteInfo(int hDb, short Count, short Options, int hInBuf, LongByReference retSize,
			IntByReference rethOutBuf);

	public short b64_NSFDbGetMultNoteInfo(long hDb, short Count, short Options, long hInBuf, LongByReference retSize,
			LongByReference rethOutBuf);

	public short b32_NSFDbGetNoteInfoExt(int hDB, int NoteID, NotesOriginatorId retNoteOID, NotesTimeDate retModified,
			ShortByReference retNoteClass, NotesTimeDate retAddedToFile, ShortByReference retResponseCount,
			IntByReference retParentNoteID);

	public short b64_NSFDbGetNoteInfoExt(long hDB, int NoteID, NotesOriginatorId retNoteOID, NotesTimeDate retModified,
			ShortByReference retNoteClass, NotesTimeDate retAddedToFile, ShortByReference retResponseCount,
			IntByReference retParentNoteID);

	public short b32_NSFNoteVerifySignature(int hNote, Memory SignatureItemName, NotesTimeDate retWhenSigned,
			Memory retSigner, Memory retCertifier);

	public short b64_NSFNoteVerifySignature(long hNote, Memory SignatureItemName, NotesTimeDate retWhenSigned,
			Memory retSigner, Memory retCertifier);
	...
	public short NSFDbCreateExtended(Memory pathName, short DbClass, boolean ForceCreation, short Options,
			byte EncryptStrength, long MaxFileSize);

	/*
	 * Define NSF DB Classes - These all begin with 0xf000 for no good reason
	 * other than to ENSURE that callers of NSFDbCreate call the routine with
	 * valid parameters, since in earlier versions of NSF the argument to the
	 * call was typically 0.
	 */

	/*
	 * The type of the database is determined by the filename extension. The
	 * extensions and their database classes are .NSX (NSFTESTFILE), .NSF
	 * (NOTEFILE), .DSK (DESKTOP), .NCF (NOTECLIPBOARD), .NTF (TEMPLATEFILE),
	 * .NSG (GIANTNOTEFILE), .NSH (HUGENOTEFILE), NTD (ONEDOCFILE), NS2
	 * (V2NOTEFILE), NTM (ENCAPSMAILFILE).
	 */
	...
	/**
	 * A database that contains every type of note (forms, views, ACL, icon,
	 * etc.) except data notes.
	 */
	 ...
	 * Release 3 when the size of a previous version of a database had been
	 * limited to 200 MB.
	 */
	...
	/**
	 * One document database with size up to 10MB. Specifically used by
	 * alternate mail to create an encapsulated database. Components of the
	 * document are further limited in size. It is not recommended that you use
	 * this database class with NSFDbCreate. If you do, and you get an error
	 * when saving the document, you will need to re-create the database using
	 * DBCLASS_NOTEFILE.
	 */
	...
	/**
	 * Specifically used by alternate mail. Not recomended for use with
	 * NSFDbCreate.
	 */
	...
	/**
	 * Database was created specifically for Domino and Notes Release
	 * Notes/Domino 6.
	 */
	...
	/*
	 * Class definitions. Classes are defined to be the "generic" classes of
	 * data type that the internal formula computation mechanism recognizes when
	 * doing recalcs.
	 */

	public static final int CLASS_NOCOMPUTE = 0 << 8;
	public static final int CLASS_ERROR = 1 << 8;
	public static final int CLASS_UNAVAILABLE = 2 << 8;
	public static final int CLASS_NUMBER = 3 << 8;
	public static final int CLASS_TIME = 4 << 8;
	public static final int CLASS_TEXT = 5 << 8;
	public static final int CLASS_FORMULA = 6 << 8;
	public static final int CLASS_USERID = 7 << 8;

	public static final int CLASS_MASK = 0xff00;
	...
	/**
	 * This item is stored in the note's summary buffer. Summary items may be
	 * used in view columns, selection formulas, and @-functions. Summary items
	 * may be accessed via the SEARCH_MATCH structure provided by NSFSearch or
	 * in the buffer returned by NIFReadEntries. API program may read, modify,
	 * and write items in the summary buffer without opening the note first. The
	 * maximum size of the summary buffer is 32K. Items of TYPE_COMPOSITE may
	 * not have the ITEM_SUMMARY flag set.
	 */
	...
	/**
	 * This is a Reader Names field. Indicated by the READER-ACCESS flag. Item
	 * is TYPE_TEXT or TYPE_TEXT_LIST.
	 */
	...
	public static final int DT_SHOWDATE = 0x0008;
	public static final int DT_SHOWABBREV = 0x0800;
	public static final int DT_STYLE_MDY = 2;
	public static final int DT_USE_TFMT = 0x0001;
	public static final int SECS_IN_WEEK = 604800;
	public static final int DT_STYLE_MSK = 0x000f0000;
	public static final int DT_STYLE_YMD = 1;
	public static final int DT_STYLE_DMY = 3;
	public static final int DT_SHOWTIME = 0x0004;
	public static final int TICKS_IN_MINUTE = 6000;
	public static final int DT_4DIGITYEAR = 0x0001;
	public static final int DT_24HOUR = 0x0040;
	public static final int SECS_IN_MONTH = 2592000;
	public static final int DT_ALPHAMONTH = 0x0002;
	public static final int TICKS_IN_DAY = 8640000;
	public static final int SECS_IN_DAY = 86400;
	public static final int TICKS_IN_SECOND = 100;
	public static final int TICKS_IN_HOUR = 360000;
	public static final int DT_VALID = 0x8000;
	...
	public short b32_NSFSearch(int hDB, int hFormula, Memory viewTitle, short SearchFlags, short NoteClassMask,
			NotesTimeDate Since, b32_NsfSearchProc enumRoutine, Pointer EnumRoutineParameter, NotesTimeDate retUntil);

	public short b64_NSFSearch(long hDB, long hFormula, Memory viewTitle, short SearchFlags, short NoteClassMask,
			NotesTimeDate Since, b64_NsfSearchProc enumRoutine, Pointer EnumRoutineParameter, NotesTimeDate retUntil);

	public short b64_NSFSearchWithUserNameList(long hDB, long hFormula, Memory ViewTitle, short SearchFlags,
			short NoteClassMask, NotesTimeDate Since, b64_NsfSearchProc EnumRoutine, Pointer EnumRoutineParameter,
			NotesTimeDate retUntil, long nameList);

	public short b32_NSFSearchWithUserNameList(int hDB, int hFormula, Memory ViewTitle, short SearchFlags,
			short NoteClassMask, NotesTimeDate Since, b32_NsfSearchProc EnumRoutine, Pointer EnumRoutineParameter,
			NotesTimeDate retUntil, long nameList);

	public interface b32_NsfSearchProc
			extends Callback { /* StdCallCallback if using __stdcall__ */
		void invoke(Pointer enumRoutineParameter, NotesSearchMatch32 searchMatch, NotesItemTable summaryBuffer);
	}

	public interface b64_NsfSearchProc
			extends Callback { /* StdCallCallback if using __stdcall__ */
		void invoke(Pointer enumRoutineParameter, NotesSearchMatch64 searchMatch, NotesItemTable summaryBuffer);
	}

	public interface NoteExtractCallback
			extends Callback { /* StdCallCallback if using __stdcall__ */
		short invoke(Pointer data, int length, Pointer param);
	}

	public interface NoteNsfItemScanProc
			extends Callback { /* StdCallCallback if using __stdcall__ */
		void invoke(short spare, short itemFlags, Pointer name, short nameLength, Pointer value, int valueLength,
				Pointer routineParameter);
	}

	public short b64_NSFNoteCipherExtractWithCallback(long hNote, NotesBlockId.ByValue bhItem, int ExtractFlags,
			int hDecryptionCipher, NoteExtractCallback pNoteExtractCallback, Pointer pParam, int Reserved,
			Pointer pReserved);

	public short b32_NSFNoteCipherExtractWithCallback(int hNote, NotesBlockId.ByValue bhItem, int ExtractFlags,
			int hDecryptionCipher, NoteExtractCallback pNoteExtractCallback, Pointer pParam, int Reserved,
			Pointer pReserved);

	public short b64_NSFFormulaCompile(Memory FormulaName, short FormulaNameLength, Memory FormulaText,
			short FormulaTextLength, LongByReference rethFormula, ShortByReference retFormulaLength,
			ShortByReference retCompileError, ShortByReference retCompileErrorLine,
			ShortByReference retCompileErrorColumn, ShortByReference retCompileErrorOffset,
			ShortByReference retCompileErrorLength);

	public short b32_NSFFormulaCompile(Memory FormulaName, short FormulaNameLength, Memory FormulaText,
			short FormulaTextLength, IntByReference rethFormula, ShortByReference retFormulaLength,
			ShortByReference retCompileError, ShortByReference retCompileErrorLine,
			ShortByReference retCompileErrorColumn, ShortByReference retCompileErrorOffset,
			ShortByReference retCompileErrorLength);

	public short b64_NSFComputeStart(short Flags, Pointer lpCompiledFormula, LongByReference rethCompute);

	public short b32_NSFComputeStart(short Flags, Pointer lpCompiledFormula, IntByReference rethCompute);

	public short b64_NSFComputeStop(long hCompute);

	public short b32_NSFComputeStop(int hCompute);

	public short b64_NSFComputeEvaluate(long hCompute, long hNote, LongByReference rethResult,
			ShortByReference retResultLength, IntByReference retNoteMatchesFormula,
			IntByReference retNoteShouldBeDeleted, IntByReference retNoteModified);

	public short b32_NSFComputeEvaluate(int hCompute, int hNote, IntByReference rethResult,
			ShortByReference retResultLength, IntByReference retNoteMatchesFormula,
			IntByReference retNoteShouldBeDeleted, IntByReference retNoteModified);
	 ...
	 * 
	 * List of security checks enabled by this flag: Restricted/unrestricted
	 * agent Can create databases Is agent targeted to this machine Is user
	 * allowed to access this machine Can user run personal agents
	 */
	...
	public short b32_AgentCreateRunContext(int hAgent, Pointer pReserved, int dwFlags, IntByReference rethContext);

	public short b64_AgentCreateRunContext(long hAgent, Pointer pReserved, int dwFlags, LongByReference rethContext);
	...
	/*
	 * allow api users to set time execution limit. if not set, default is 0
	...
	/*
	 * Definitions for stdout redirection types. This specifies where output
	 * from the LotusScript "print" statement will go
	 */

	public static short AGENT_REDIR_NONE = 0; /* goes to the bit bucket */
	public static short AGENT_REDIR_LOG = 1; /*
												 * goes to the Notes log (default)
												 */
	public static short AGENT_REDIR_MEMORY = 2; /*
												 * goes to a memory buffer,
												 * cleared each AgentRun
												 */
	public static short AGENT_REDIR_MEMAPPEND = 3; /*
													 * goes to buffer, append
													 * mode for each agent
													 */
	 ...
	 * database with the privileges of the signer of the agent. If the flag is
	 * not set, the agent's "context" database will be open with the privileges
	 * of the current user (meaning, the current Notes id, or the current Domino
	 * web user).
	 */

	public short b32_AgentRun(int hAgent, int hAgentCtx, int hSelection, int dwFlags);

	public short b64_AgentRun(long hAgent, long hAgentCtx, int hSelection, int dwFlags);
	...
	/**
	 * note has no purge status. Returned only when SEARCH_FULL_DATACUTOFF is
	 * used
	 */
	...
	/**
	 * if there is reader's field at doc level this is the return value so that
	 * we could mark the replication as incomplete
	 */
	public static byte SE_FNOACCESS = 0x20;
	/**
	 * note has truncated attachments. Returned only when SEARCH1_ONLY_ABSTRACTS
	 * is used
	 */
	...
	/**
	 * DTF - Any .DTF. Used for container and sort temp files to give them a
	 * more unique name than .TMP so we can delete *.DTF from the temp directory
	 * and hopefully not blow away other application's temp files.
	 */
	...
	public short OSPathNetConstruct(Memory PortName, Memory ServerName, Memory FileName, Memory retPathName);

	public short OSPathNetParse(Memory PathName, Memory retPortName, Memory retServerName, Memory retFileName);

	public short b32_OSMemAlloc(short BlkType, int dwSize, IntByReference retHandle);

	public short b64_OSMemAlloc(short BlkType, int dwSize, LongByReference retHandle);
	...
	/*
	 * These must be OR-ed into the ObjectType below in order to get the desired
	 * behavior. Note that OBJECT_COLLECTION implicitly has both of these bits
	 * implied, because that was the desired behavior before these bits were
	 * invented.
	 */
