...
import com.mindoo.domino.jna.internal.NotesCallbacks.b64_DESIGNENUMPROC;
import com.mindoo.domino.jna.internal.NotesCallbacks.b64_FPMailNoteJitEx2CallBack;
...
//	public native short CreateNamesListFromSessionID(Memory pszServerName, SESSIONID SessionId, LongByReference rtnhNames);
	...
	public native short FolderMove(long hDataDB, long hFolderDB, int FolderNoteID, long hParentDB, int ParentNoteID,
			int dwFlags);
	...
	public native short MIMEOpenDirectory(
			long hNote,
			IntByReference phMIMEDir);
	
	public native short MIMEFreeDirectory(
			long hMIMEDir);
	
	public native Pointer MIMEEntityContentID(
			Pointer pMIMEEntity);

	public native Pointer MIMEEntityContentLocation(
			Pointer pMIMEEntity);
	
	public native int MIMEEntityContentSubtype(
			Pointer pMIMEEntity);

	public native int MIMEEntityContentType(
			Pointer pMIMEEntity);
	
	public native Pointer MIMEEntityGetHeader(
			Pointer pMIMEEntity,
			int symKey);

	public native short MIMEEntityGetTypeParam(
			Pointer pMIMEEntity,
			int symParam,
			LongByReference phValue,
			IntByReference pdwValueLen);
	
	public native boolean MIMEEntityIsDiscretePart(
			Pointer pMimeEntity);

	public native boolean MIMEEntityIsMessagePart(
			Pointer pMIMEEntity);
	
	public native boolean MIMEEntityIsMultiPart(
			Pointer pMimeEntity);

	public native short MIMEFreeEntityDataObject(
			long hNote,
			Pointer pEntity);

	public native short MIMEGetDecodedEntityData(
			long hNote,
			Pointer pMimeEntity,
			int dwEncodedOffset,
			int dwChunkLen,
	...
	public native void  MimeGetExtFromTypeInfo(
			Memory pszType,
			Memory pszSubtype,
			Memory pszExtBuf,
			short wExtBufLen,
	...
	public native void MIMEGetFirstSubpart(
			int  hMIMEDir,
			Pointer  pMIMEEntity,
			PointerByReference retpMIMEEntity);
	
	public native short MIMEGetNextSibling(
			int hMIMEDir,
			Pointer pMIMEEntity,
			PointerByReference retpMIMEEntity);

	public native short MIMEGetParent(
			int  hMIMEDir,
			Pointer  pMIMEEntity,
			PointerByReference retpMIMEEntity);
	
	public native short MIMEGetPrevSibling(
			int  hMIMEDir,
			Pointer  pMIMEEntity,
			PointerByReference retpMIMEEntity);
	...
	public native short MIMEHeaderNameToItemName(
			short wMessageType,
	...
	public native short MIMEItemNameToHeaderName(
			short wMessageType,
			Memory pszItemName,
	...
	public native int MIMEStreamGetLine(
			Memory pszLine,
			int uiMaxLineSize,
			Pointer hMIMEStream);
	...
	public native short MIMEConvertCDParts(
			long hNote,
			boolean bCanonical,
			boolean bIsMIME,
			Pointer hCC);
	...
	@Override
	public native short NSFNoteReplaceItems(long hSrcNote, long hDstNote, ShortByReference pwRetItemReplaceCount,
			boolean fAllowDuplicates);

	@Override
	public native short StoredFormAddItems(long hSrcDbHandle, long hSrcNote, long hDstNote, boolean bDoSubforms, int dwFlags);

	@Override
	public native short StoredFormRemoveItems(long hNote, int dwFlags);

	@Override
	public native short MailNoteJitEx2(Pointer vpRunCtx, long hNote, short wMailFlags, IntByReference retdwRecipients,
			short jitflag, short wMailNoteFlags, b64_FPMailNoteJitEx2CallBack vCallBack, Pointer vCallBackCtx);
	
	@Override
	public native short MailSetSMTPMessageID(long hNote, Memory domain, Memory string, short stringLength);
	
	@Override
	public native short NSFDbReopenWithFullAccess(long hDb, LongByReference hReopenedDb);
	...
	@Override
	public native short REGCrossCertifyID(long hCertCtx, short spare1, Memory regServer, Memory idFileName, Memory location,
	...
	public native short SECKFMGetCertifierCtx(Memory pCertFile, ByReference pKfmPW, Memory pLogFile,
			...
			LongByReference rethKfmCertCtx, ShortByReference retfIsHierarchical, ShortByReference retwFileVersion);

	@Override
	public native void SECKFMFreeCertifierCtx(long hKfmCertCtx);

	@Override
	public native short SECKFMMakeSafeCopy(long hKFC, short Type, short Version, Memory pFileName);
	...
	@Override
	public native short DesignGetNoteTable(long hDB, short NoteClass, LongByReference rethIDTable);
}
